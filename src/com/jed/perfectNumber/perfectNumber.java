...
public class perfectNumber {
  ...
  public static boolean isPerfectNumber(int number) {
    ...
    if (number > 1) {
      for (int i = 1; i < number; i++) { //as long as i is less than number keep looping
        perfectNumber += i;
        if (number % i == 0 && number == perfectNumber) { // if number is not a proper divisor or does not equal the
          // perfect number
